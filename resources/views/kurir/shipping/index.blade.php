    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div>
                ...
                <span class="badge bg-info">Halaman: {{ $shipments->currentPage() }} dari {{ $shipments->lastPage() }}</span>
            </div>
        </div>
        <div class="card-body">
                ...
                <table class="table table-hover align-middle" id="shipmentsTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $shipment->kode_transaksi ?? $shipment->kode_transaksi_full ?? '-' }}</td>
                            <td>
                                <div><strong>{{ $shipment->nama_pelanggan ?? 'N/A' }}</strong></div>
                                <small class="text-muted">{{ $shipment->email_pelanggan ?? '-' }}</small>
                            </td>
                            <td>
                                <div>{{ $shipment->nama_penerima ?? 'N/A' }}</div>
                            ...
                            <td>
                                ...
                                @else
                                    <span class="badge bg-secondary">{{ ucfirst(str_replace('_', ' ', $shipment->status_pengiriman ?? 'Unknown')) }}</span>
                                @endif
                            </td>
                            <td>
                                @if($shipment->id_kurir && $shipment->nama_kurir)
                                    <div>
                                    ...
                                    @if($shipment->phone_kurir)
                                        <div><small class="text-muted"><i class="bi bi-phone"></i> {{ $shipment->phone_kurir }}</small></div>
                                    @endif
                                @else
                            ...
                            <td>
                                @if(!$shipment->id_kurir)
                                    ...
                                    @if(in_array($shipment->status_pengiriman, ['pending', 'dikemas', 'siap_diambil']))
                                        <form action="{{ route('kurir.shipping.assign', $shipment->id_pengiriman) }}" method="POST" class="d-inline">
                                            @csrf
                                    ...
                                    @else
                                        <span class="text-muted"><small>Status: {{ $shipment->status_pengiriman }}</small></span>
                                    @endif
                                @else
                                    {{-- Sudah assign - tampilkan tombol unassign jika belum dalam pengiriman --}}
                                    @if(in_array($shipment->status_pengiriman, ['pending', 'dikemas', 'siap_diambil']))
                                        <form action="{{ route('kurir.shipping.unassign', $shipment->id_pengiriman) }}"
                                              method="POST"
                                              class="d-inline"
                                    ...
                                    @elseif(in_array($shipment->status_pengiriman, ['dalam_pengiriman', 'terkirim']))
                                        <span class="badge bg-secondary">
